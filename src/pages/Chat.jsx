...
function getHelperReply(helper) {
  const map = {
    logopedia: "¡Hola! Claro, tengo disponibilidad esta semana. ¿Puedes contarme un poco más sobre las dificultades? Así te digo si es mi especialidad.",
    ...
    cuidado: "Buenas, con mucho gusto. ¿Me puedes contar un poco más sobre tu padre? ¿Tiene movilidad reducida o necesidades especiales?",
    mascotas: "¡Hola! ¿Qué raza tiene tu perro? ¿Necesita paseos o cuidado en casa durante el fin de semana?",
    matematicas: "¡Hola! Doy clases presenciales o por videollamada. ¿En qué curso está tu hijo y qué temas le cuestan más?",
    entrenador: "¡Hola! Perfecto. La primera sesión es gratuita para evaluar objetivos. ¿Te viene bien esta semana?",
  }
...
export default function Chat() {
  ...
  useEffect(() => {
    if (helper) setSuggested(generateFirstMessage(helper))
  }, [helper])
    ...
    <div className={styles.page}>
      ...
      <div className={styles.messages}>
        <div className={styles.systemNote}>
          <Shield size={11} />
          <span>Chat seguro · Los datos personales se comparten solo cuando tú decides</span>
        </div>
        ...
        {messages.reduce((acc, msg, i) => {
          ...
          const showDate = !prev || formatDate(msg.time) !== formatDate(prev.time)
          return [...acc,
            showDate && <div key={`date-${i}`} className={styles.dateLabel}>{formatDate(msg.time)}</div>,
