            ...
            <div key={h.id} className={styles.card} onClick={() => navigate(`/helper/${h.id}`, { state: { helper: h } })}>
              <div className={styles.cardTop}>
                <div className={styles.avatarWrap}>
                  ...
                  {h.urgent && <span className={styles.urgentBadge}><Zap size={9} /></span>}
                </div>
              </div>
              <div className={styles.cardBody}>
                ...
                <div className={styles.cardMeta}>
                  ...
                  <MapPin size={10} /> {h.distance}km
                </div>
              </div>
              <div className={styles.cardFooter}>
                <span className={styles.price}>{h.price && h.price !== 'Consultar' ? h.price : 'Consultar'}</span>
                <div className={styles.cardBadges}>
                  {h.presential && <span className={styles.badge}>📍</span>}
                  {h.online && <span className={styles.badge}>💻</span>}
                </div>
              </div>
            </div>
          ))}
